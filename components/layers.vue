...
const diagWater = (mask, x, y, dx, dy, rows, cols) => {
  const nx = x + dx;
  const ny = y + dy;
  return !inBounds(nx, ny, rows, cols) || mask[nx - 1][ny - 1] === 0;
};
...
const classifyMask = (mask) => {
  ...
  for (let x = 1; x <= rows; x++)
    for (let y = 1; y <= cols; y++) {
      ...
      const w = exposures(mask, x, y, rows, cols);
      const cardMask =
        (w.north << 0) | (w.east << 1) | (w.south << 2) | (w.west << 3);
      const entry = cardinalLookup[cardMask] || cardinalLookup[0];

      const diagState = {
        ne: diagWater(mask, x, y, diagInfo.ne.dx, diagInfo.ne.dy, rows, cols),
        se: diagWater(mask, x, y, diagInfo.se.dx, diagInfo.se.dy, rows, cols),
        sw: diagWater(mask, x, y, diagInfo.sw.dx, diagInfo.sw.dy, rows, cols),
        nw: diagWater(mask, x, y, diagInfo.nw.dx, diagInfo.nw.dy, rows, cols),
      };
      ...
      switch (entry.kind) {
        ...
        case "opposite": {
          ...
          const dirName =
            options.find((name) => w[name]) || options[0] || "south";
          tileShape = "ramp";
...
const maskHasFilledCell = (mask) =>
  mask.some((row) => row.some((value) => value === 1));

const buildMaskForLevel = (heights, level) =>
  heights.map((row) => row.map((value) => (value >= level ? 1 : 0)));

const cleanupStackedSlopes = (voxels, rows, cols, options = {}) => {
  ...
  for (let layer = 1; layer < voxels.length; layer++) {
    ...
    for (const key of Object.keys(current)) {
      ...
      const supportBaseShape =
        support.cliffBaseShape ?? (supportShape === "cliff" ? supportVariant : supportShape);
  ...
  methods: {
    ...
    rebuildTerrainFromState(state) {
      ...
      for (let level = 0; level <= maxHeight; level++) {
        ...
        const { shape, rot } = classifyMask(mask);
        for (let x = 1; x <= rows; x++) {
          for (let y = 1; y <= cols; y++) {
            if (mask[x - 1][y - 1] !== 1) continue;
